...
<div class="container-fluid">
	...
	<div class="container-fluid">
		<div class="panel">
			<div class="panel-heading">
				<h4>Rekap Akreditasi Nasional</h4>
			</div>
							...
							<div class="panel-heading">
								<h1>
									...
									<span class="pull-right">
										...
										<?php 
										$unggul = mysqli_query($koneksi,"select * from tb_akreditasi where akre_status='1'");
	...
	<div class="container-fluid">
		<div class="panel">
			<div class="panel-heading">
				<h4>Pantauan Proses Akreditasi Program Studi</h4>
			</div>
			<div class="panel-body">
				<table id="table" class="table table-bordered table-striped table-hover table-datatable">
					<thead>	
						<tr>
							...
							<th>Batas Berlaku</th>
							<th>Pantauan Peringatan</th>									
						</tr>
					</thead>
					<tbody>
						<?php 
						$datenow = date ('Y-m-d');				
						$data = mysqli_query($koneksi, "select * from tb_akreditasi, tb_fakultas, tb_jenjang, tb_prodi, tb_statusakre where akre_fakultas=fakultas_id && akre_jenjang=jenjang_id && akre_prodi=prodi_id && akre_status = status_id && datediff(akre_tglakhir, CURDATE()) < 733");
						$no = 1;
						...
						while($d=mysqli_fetch_array($data)){
							...
							<tr>
								...
								<td>
									<?php 
									if($jarak->days<="366" && $jarak->days>="181"){
										echo "<div class='label label-warning'>SUSUN LED</div>";
									}else if($jarak->days<="180" && $jarak->days>="1"){
										echo "<div class='label label-danger'>KIRIM LED</div>";
									}else if($jarak->days<="733" && $jarak->days>="367"){
										echo "<div class='label label-info'>PANTAUAN REAKREDITASI</div>";
									}else if($jarak->days<="0"){
										echo "<div class='label label-info'>EXPIRED</div>";
									}
									?>							
								</td>	
	...
	<div class="container-fluid">
		<div class="panel">
			<div class="panel-heading">
				<h4>Riwayat Akreditasi Nasional Program Studi</h4>
			</div>
			<div class="panel-body">
				<table id="table" class="table table-bordered table-striped table-hover table-datatable">
					<thead>	
					...
					<tbody>
						<?php 				
						$data = mysqli_query($koneksi, "select * from tb_akreditasi, tb_fakultas, tb_jenjang, tb_prodi, tb_statusakre where akre_fakultas=fakultas_id && akre_jenjang=jenjang_id && akre_prodi=prodi_id && akre_status = status_id && akre_kategori ='1'");
						$no = 1;
						// mengubah data ke array dan menampilkannya dengan perulangan while
						while($d=mysqli_fetch_array($data)){
							$tgltoday = date ('Y-m-d');
